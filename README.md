...
- **Shortcut Desktop cho ban phan phoi:** cach chac chan nhat, khong can
  chay script gi ca: chuot phai vao `ME Launcher.exe` trong
  `build/dist/ME Launcher/` -> **Send to -> Desktop (create shortcut)**.
  Ngoai ra co 2 file tu dong trong `packaging/` (copy vao
  `build/dist/ME Launcher/` moi lan `gradle jpackageImage`):
  `Tao-shortcut-Desktop.vbs` va `Tao-shortcut-Desktop.bat` - ban .bat
  goi PowerShell nen it bi chan boi chinh sach Windows Script Host hon
  ban .vbs, nhung ca 2 deu co the bi Windows SmartScreen canh bao vi la
  file tai tu mang ve. Neu ca 2 deu khong chay duoc, dung cach thu cong
  o tren, luon chay duoc 100%.
